...
class MakeReportCommand extends Command
{
    ...
    public function fire()
    {
        ...
        $this->line('');
        $this->info('Next: fill repository SQL, add routes in your domain route file, run tests.');
        $this->comment('Suggested routes:');
        $this->line("  Route::get('{$route}', 'Reports\\\\{$studly}ReportController@index');");
        if (!empty($flags['datatables'])) {
            $this->line("  Route::get('{$route}/data', 'Reports\\\\{$studly}ReportController@data');");
        }
